...
new_testing_features = list(line for line in new_test_words)
new_testing_labels = list(line.replace("\n", "") for line in new_test_labels)
##print(new_testing_features)
##print(new_testing_labels)
...
for trainingIndexes, testingIndexes in sss:
    ...
    for batch, batch_label in zip(newTrainingData, newTrainingLabel):
        ...
        X_new_tfidf = tf_idf_transformer.transform(count_vect.transform(new_testing_features))
        estimated.append(algorithm.predict(count_vect.transform(new_testing_features)))

    new_predicted = voting(estimated)
    metric = precision_recall_fscore_support(new_testing_labels, new_predicted)
    accuracy += metrics.accuracy_score(new_testing_labels, new_predicted, normalize=True, sample_weight=None)
